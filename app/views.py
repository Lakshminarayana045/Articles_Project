    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user:
                ...
                return render(request, 'login.html',{'message':'Username and Password are not matching ..!!'})
    else:
        form = UserLoginForm()
...
def login_articles(request):
    userid = request.user.id
    data = Post.objects.filter(author_id=userid)
    return render(request,'login_articles.html',{'data':data})
